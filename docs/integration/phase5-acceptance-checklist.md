...
> 当前补充：上方证据为历史阶段记录；`2026-04-18` 再次复核时，当前 `main` 已达到 `41` 个测试文件、`227` 个测试全部通过，可作为论文与答辩阶段的最新口径。
...
> 当前补充：上方证据为历史阶段记录；`2026-04-18` 再次复核时，当前 `main` 的 `npm run build` 已成功完成，且未再出现 chunk 大小告警。
...
- 是否已验证：待补充
- 未验证原因：若本轮未做实机摄像头联调，应明确记录“仅完成代码、文档与静态验证，尚未做目标电脑实机补证”。
- 必须至少覆盖的子证据：浏览器摄像头权限授权、活体挑战通过、人脸录入成功、人脸预检成功、正式打卡成功、业务记录页可查看 `FACE_LIVENESS_*` 日志、统计页可查看运行事件概况。
- 若已验证，证据放在哪里（按第 6 节格式填写）：
- 补充说明：本项用于证明系统仅依赖一台带普通 RGB 摄像头的电脑即可完成人脸、活体与打卡闭环，不得把外接考勤机、红外/深度相机或其他外设作为前提。

#### 5.2.7 活体阈值调优记录

- 是否已验证：待补充
- 未验证原因：若尚未在目标电脑上做多轮活体挑战，应明确记录“当前仅使用默认阈值，未完成机器实测校准”。
- 必须至少覆盖的子证据：测试环境、浏览器版本、摄像头类型、通过样本数量、失败样本数量、主要失败原因、最终采用的阈值或保留默认值理由。
- 若已验证，证据放在哪里（按第 6 节格式填写）：
- 补充说明：建议与 `docs/integration/phase5-local-runbook.md` 中的“活体阈值调优记录模板”一起使用，避免后续换电脑后重复摸索。
...
- 单机摄像头方案补证至少再增加 3 份截图或等价录屏片段：摄像头权限授权/画面、活体挑战通过结果、人脸录入或打卡成功后的页面结果。
...
#### 已知问题 2：前端构建曾存在 chunk 大小告警（当前复核已消失）
...
- 当前结论：该问题在 `2026-04-07` 阶段为非阻塞已知项；经 `2026-04-18` 再次复核，当前 `main` 构建已不再出现该告警，历史记录仅保留用于回溯。
- 是否阻塞最终通过判定：否
- 临时处理或绕过方式：历史阶段为保留记录；当前无需绕过。
- 证据位置：`前端构建 | 2026-04-07 17:43 / frontend | Vite build 输出（成功，存在 chunk 大小告警）`；`前端构建复核 | 2026-04-18 / frontend | npm run build 输出（成功，未出现 chunk 大小告警）`
- 后续跟进说明：若后续再次出现同类告警，应以最新构建输出重新登记，不再沿用本历史问题结论。
